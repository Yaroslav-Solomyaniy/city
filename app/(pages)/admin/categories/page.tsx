...
import {
    ...
    ChevronDown, Check, Layers, ChevronRight, Icon, SelectedIcon
} from 'lucide-react'
                    ...
                    <table className="w-full border-collapse min-w-160">
                        ...
                        {filtered.length === 0 && (
                            <tr><td colSpan={6} className="text-center py-16">
                                ...
                                <button onClick={() => setSearch('')} className="text-[13px] font-medium" style={{ color: 'var(--text-accent)' }}>Скинути пошук</button>
                            </td></tr>
                        )}
                        {filtered.map(cat => {
                                ...
                                <tr key={cat.id}
                                    ...
                                    <td className="px-5 py-3.5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0">
                                                <p className="font-semibold text-[13.5px] m-0 leading-tight" style={{ color: 'var(--text-primary)' }}>{cat.title}</p>
                                    ...
                                    <td className="px-5 py-3.5 hidden md:table-cell">
                                        <div className="flex flex-wrap gap-1">
                                            ...
                                            {cat.services.length > 2 && (
                                                <span className="text-[11px] px-2 py-0.5 rounded-full" style={{ color: 'var(--text-muted)' }}>+{cat.services.length - 2}</span>
                                            )}
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                                        <div className="flex items-center gap-1">
                                            {/* Arrow hint on hover */}
                                            <ChevronRight size={15} className="opacity-0 group-hover:opacity-40 transition-opacity mr-1"
...
function CategoryFormModal({ category, onSave, onClose }: {
    ...
    const submit = (e?: React.FormEvent) => {
        e?.preventDefault()
        onSave({ ...(category ?? {}), title, titleEn, iconName, photo, accent, bg,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div className="w-full max-w-xl max-h-[90vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden"
                ...
                <div className="flex items-center justify-between px-6 py-4 shrink-0"
                    ...
                    <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg" style={{ color: 'var(--text-muted)' }}>
                        <X size={18} />
                    </button>
                </div>
                ...
                <form onSubmit={submit} className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className={labelCls} style={{ color: 'var(--text-muted)' }}><Type size={10} className="inline mr-1" />Назва (UA)</label>
                            <input required value={title} onChange={e => setTitle(e.target.value)} placeholder="Здоров'я" className={inputCls} style={inputSt} />
                        </div>
                        <div>
                            <label className={labelCls} style={{ color: 'var(--text-muted)' }}><Globe size={10} className="inline mr-1" />Назва (EN)</label>
                            <input required value={titleEn} onChange={e => setTitleEn(e.target.value)} placeholder="Health" className={inputCls} style={inputSt} />
                        </div>
                    </div>
                        ...
                        <div className="relative">
                            ...
                            {iconOpen && (
                                <>
                                    ...
                                    <div className="absolute left-0 right-0 top-full mt-1 z-20 rounded-xl p-2 shadow-xl max-h-44 overflow-y-auto grid grid-cols-4 gap-1"
                                         style={{ background: 'var(--bg-card)', border: '1px solid var(--border-card)' }}>
                                        {ICON_OPTIONS.map(opt => {
                                            const OI = opt.icon; const sel = opt.name === iconName
                                            return (
                                                <button key={opt.name} type="button"
                                                        ...
                                                        style={sel ? { background: 'var(--admin-nav-active-bg)', color: 'var(--admin-nav-active-text)' } : { color: 'var(--text-secondary)' }}
                                                        onMouseEnter={e => { if (!sel) (e.currentTarget as HTMLElement).style.background = 'var(--admin-nav-hover-bg)' }}
                                                        onMouseLeave={e => { if (!sel) (e.currentTarget as HTMLElement).style.background = 'transparent' }}>
                                                    <OI size={18} /><span className="text-[9px] font-medium">{opt.name}</span>
                                                </button>
                                            )
                                        })}
                                    </div>
                    ...
                    <div>
                        <label className={labelCls} style={{ color: 'var(--text-muted)' }}><ImageIcon size={10} className="inline mr-1" />URL фото</label>
                        <input value={photo} onChange={e => setPhoto(e.target.value)} placeholder="https://images.unsplash.com/..." className={inputCls} style={inputSt} />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className={labelCls} style={{ color: 'var(--text-muted)' }}><Palette size={10} className="inline mr-1" />Акцент</label>
                            <div className="flex gap-2">
                                <input type="color" value={accent} onChange={e => setAccent(e.target.value)} className="w-9 h-[42px] rounded-lg cursor-pointer border-0 bg-transparent" />
                                <input value={accent} onChange={e => setAccent(e.target.value)} className={`${inputCls} flex-1`} style={inputSt} />
                            </div>
                        </div>
                        <div>
                            <label className={labelCls} style={{ color: 'var(--text-muted)' }}><Palette size={10} className="inline mr-1" />Фон іконки</label>
                            <div className="flex gap-2">
                                <input type="color" value={bg} onChange={e => setBg(e.target.value)} className="w-9 h-[42px] rounded-lg cursor-pointer border-0 bg-transparent" />
                                <input value={bg} onChange={e => setBg(e.target.value)} className={`${inputCls} flex-1`} style={inputSt} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input value={services} onChange={e => setServices(e.target.value)} placeholder="Лікарні, Поліклініки, Аптеки" className={inputCls} style={inputSt} />
                ...
                {/* Footer */}
                <div className="flex gap-3 justify-end px-6 py-4 shrink-0" style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <button onClick={() => submit()} className="px-5 py-2.5 rounded-xl text-[13px] font-semibold text-white hover:opacity-90"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div className="w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden"
                ...
                <div className="p-6 text-center">
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: 'var(--admin-danger-bg)' }}>
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={onConfirm} className="flex-1 px-4 py-2.5 rounded-xl text-[13px] font-semibold text-white hover:opacity-90"
